...
export class ShopifyService {
  ...
  private readonly seccionLandingLiquid = [
    ...
    // comportamiento, no una copia nuestra. Se reintenta durante 3 segundos
    // (cada 200ms) porque Releasit puede tardar un momento en crear su botón
    // real después de que esta página ya cargó — si a los 3 segundos
    // Releasit todavía no lo creó (o no está instalado en esta tienda, o
    // cambió ese id en una actualización futura), se deja de intentar y el
    // botón que armamos nosotros se queda tal cual, con su respaldo de
    // siempre (clickear al real si aparece más tarde, o agregar al carrito).
    // Solo se mueve UNO (Releasit solo crea un botón real por página): si la
    // landing tiene varios "COMPRAR AHORA" agregados, se lleva el real el
    // primero que aparece en la página (ver ecomMagnatesRsiTarget arriba).
    '<script>',
    '(function(){',
